...
interface ChatMessage {
  text: string;
  sender: 'user' | 'bot'; // Specify that sender can only be 'user' or 'bot'
}
interface Progress {
  HTML: number;
  CSS: number;
...
const api = {
  ...
  saveProgress: async (progress: object) => {
    await new Promise(resolve => setTimeout(resolve, 1000));
...
const generateQuizQuestion = (language: string | null) => {
  ...
  return questions[language as keyof typeof questions][Math.floor(Math.random() * questions[language as keyof typeof questions].length)];
...
export default function EnhancedLearningPlatform() {
  ...
  const runCode = async () => {
    if (selectedLanguage) {
      ...
      // Optionally handle the case where no language is selected
  ...
  const handleQuizAnswer = async (answerIndex: number) => {
    ...
    if (answerIndex === quizQuestion.correctAnswer) {
      ...
      if (selectedLanguage) {
        const langKey = selectedLanguage as keyof typeof progress;
        const newProgress = {
          ...progress,
          [langKey]: Math.min(100, (progress[langKey] || 0) + 10)
        };
    ...
    const newQuestion = generateQuizQuestion(selectedLanguage);
    if (newQuestion) {
  ...
  const sendChatMessage = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (chatInput.trim()) {
      // Create a new messages array with the correct type
      const newMessages: ChatMessage[] = [...chatMessages, { text: chatInput, sender: 'user' }];
      setChatMessages(newMessages);
      setChatInput('');

      const response = await api.getChatbotResponse(chatInput);

      // Create the bot's response message with the correct type
            ...
            {selectedLanguage && (
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {chapters[selectedLanguage as keyof typeof chapters].map((chapter: string) => ( // Specify that chapter is a string
                    <button
                      key={chapter}
                      onClick={() => setSelectedChapter(chapter)} // This should now work
                      className={`p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow ${theme === 'dark' ? 'bg-gray-800 hover:bg-gray-700' : 'bg-white hover:bg-gray-50'}`}
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <textarea
                  ...
                  className={`w-full h-64 p-2 border rounded ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-300'
                    }`}
              ...
              <div>
                <pre className={`w-full h-64 p-2 border rounded overflow-auto ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-300'
                  }`}>
                  {output}
                </pre>
              </div>
                ...
                <div className="space-y-2">
                  {quizQuestion.options.map((option, index) => (
                    <button
                      ...
                      className={`w-full text-left p-2 rounded ${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-gray-100'
                        }`}
                    >
                      {option}
                    </button>
        ...
        return (
          <div className="space-y-8">
            ...
            {Object.entries(progress).map(([lang, prog]) => (
              <div key={lang} className={`p-6 rounded-lg shadow-md ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'
                }`}>
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
      ...
      <div className="fixed bottom-4 right-4">
        {isChatbotOpen ? (
          <div className={`w-80 h-96 rounded-lg shadow-lg flex flex-col ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'
            }`}>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {chatMessages.map((message, index) => (
                <div key={index} className={`${message.sender === 'user' ? 'text-right' : 'text-left'}`}>
                  <span className={`inline-block p-2 rounded-lg ${message.sender === 'user'
